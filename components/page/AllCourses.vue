              ...
              <div class="courses-tab-menu aos-init aos-animate" data-aos="fade-up" data-aos-duration="1000">
                  <ul class="nav justify-content-lg-end">
                      ...
                      <li v-for="(cat,key) in top_categories" :key="key">
                        <button @click="tabClick(cat.id)" :class="tab === cat.id ? 'active':''">{{ cat.name }}</button></li>
      ...
      <div class="allcoursesarea py-5"  :style="{ backgroundColor: section.bg_color }">
        ...
        <v-container class="btcontainer" v-else>
          <v-row class="justify-content-center" dense>
                <v-col cols="12" md="3" class="h-auto" v-for="(cous,key) in courses" :key="key">
                  <!-- Course Start -->
...
export default {
  name: 'AllCourse',
  components:{BreadCrumbs2,ContactForm2},
  props:{
    section:{
      required:false,
      type:Object,
  ...
  data() {
    return {
      ...
      pageInfo: {
        pageName: 'Courses',
        apiUrl: '/get/all/course?page=1&per_page=32',
      },
