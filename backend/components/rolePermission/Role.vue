    ...
    <v-card-text>
      <v-data-table
        ...
        <template v-slot:item.type="{ item }">
            <span >{{findTypeById(item.type)}}</span>
        </template>
        ...
        <template v-slot:item.actions="{ item }">
          ...
          <v-btn fab x-small color="red" v-if="$can('remove',pageInfo.permission)" class="ml-1" @click="deleteItem(item, items.indexOf(item), state.store_name)">
...
export default {
  ...
  computed: {
    ...
    types() {
      return this.$store.state.rolePermission.basic['types']
    },
    editMode() {
  ...
  methods: {
    findTypeById(id){
      let tt = this.types.find(item=>item.id===id)
