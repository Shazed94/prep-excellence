<template>
  <v-container fluid>
    <bread-crumbs
      ...
      :items="[{text: 'Test',disabled: true, href: '/employee/exam'}]"
    />
    <v-card>
      <v-data-table
        dense
              ...
              <v-card>
                <validation-observer ref="observer" v-slot="{ invalid }">
                  <v-form ref="form" @submit.prevent="submitData()">
                    <v-card-title class="text-h5"> {{ editIndex > -1 ? 'Update ' : 'Create' }} {{ pageInfo.pageName2 | capitalize }}</v-card-title>
                    <v-card-text>
                      <v-container>
                        <v-row dense>
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-if="item.exam_type === 1" x-small class="accent" v-bind="attrs"
                     v-on="on" fab :to="`/employee/question/edit/${item.id}`">
                <v-icon>mdi-eye</v-icon>
              </v-btn>
              <v-btn v-else x-small class="accent" v-bind="attrs"
                     v-on="on" fab :to="`/employee/question/edit/sat/${item.id}`">
                <v-icon>mdi-eye</v-icon>
              </v-btn>
            </template>
            <span>View</span>
...
<!--          <v-tooltip bottom v-if="$can('create',pageInfo.permission)">
            <template v-slot:activator="{ on, attrs }">
              <v-btn v-if="item.exam_type === 1" x-small class="accent" v-bind="attrs"
                     v-on="on" fab :to="`/employee/question/${item.id}`">
                <v-icon>mdi-plus-box-multiple</v-icon>
              </v-btn>
              <v-btn v-else x-small class="accent" v-bind="attrs"
                     v-on="on" fab :to="`/employee/question/sat/${item.id}`">
                <v-icon>mdi-plus-box-multiple</v-icon>
              </v-btn>
            </template>
            <span>Add More</span>
      ...
      <v-card>
        <validation-observer ref="observer" v-slot="{ invalid }">
          <v-form ref="form" @submit.prevent="submitCloneData()">
            <v-card-title class="text-h5"> Clone {{ pageInfo.pageName2 | capitalize }}</v-card-title>
            <v-card-text>
              <v-container>
                <v-row dense>
                  <v-col cols="12" xs="12" sm="4">
...
const permission = 'Employee Test'
...
export default {
  ...
  data() {
    return {
      moment,
      pageInfo: {
        pageName: 'Test Manage',
        pageName2: 'Test',
        apiUrl: '/employee/bbb-employee/exams/',
        permission,
      },
      state: {
        name: stateName,
  ...
  computed: {
    ...mapGetters('employee/exams',['exams','all_sat_sections','totalItems']),
  ...
  async mounted() {
    ...
    if (!this.all_sat_sections.length) await this.$store.dispatch('employee/exams/getItems2', payload8)
  ...
  methods: {
    async init() {
      ...
      if (!this.exams.length) await this.$store.dispatch(storeName+'/getItems', payload1)
      this.loader.isLoading = false
    },
    ...
    async submitData() {
      ...
      await this.$axios.post(url, formData).then((response) => {
        ...
        if (!this.editMode){
          ...
          if (data.exam_type === 1){
            this.$router.push(`/employee/question/${data.id}`)
          }else{
            this.$router.push(`/employee/question/sat/${data.id}`)
          }
        }else this.$toaster.success(`${this.pageInfo.pageName} ${this.editMode ? 'updated' : 'created'} successfully!!`)
