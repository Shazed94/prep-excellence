<template>
  <div class="flex-grow-1">
    <bread-crumbs
      ...
      :items="[{text: 'Accounts',disabled: true, href: '/accounts'},{text: `${pageInfo.pageName}`,disabled: true, href: '/accounts/banks'}]"
    />
    <v-data-table
      dense
      v-model="selected"
    ...
    >
      <template v-slot:top>
        ...
        >
          <v-toolbar-title>
            <div>
              <export-excel :url="`bank/export?format=excel&ids=${selectedIds ? selectedIds : ''}`" file_name="banks" />
              <export-csv :url="`bank/export?format=csv&ids=${selectedIds ? selectedIds : ''}`" file_name="bank" />
              <export-pdf  :url="`bank/export?format=pdf&ids=${selectedIds ? selectedIds : ''}`" file_name="banks" />
            </div>
          </v-toolbar-title>
...
export default {
  ...
  computed: {
    ...mapGetters('accounts/bank', ['banks']),
    selectedIds() {
  ...
  methods: {
    ...
    toggleStatus(index, value, event, id) {
      this.$axios.get(`${this.pageInfo.apiUrl}toggle/${id}`).then(response => {
        if(response.data.status === 'success') {
          this.$store.commit('accounts/bank/BANK_STATUS_CHANGE', {index, value })
    ...
    async submit() {
      ...
      await this.$axios.post(this.pageInfo.apiUrl, this.form).then(response => {
        this.dialog = false
        this.$store.commit('accounts/bank/ADD_NEW', response.data.data)
        this.clear()
    ...
    async update() {
      ...
      await this.$axios.post(`${this.pageInfo.apiUrl}${this.selectedItem.id}`, this.form).then(response => {
        this.$store.commit('accounts/bank/UPDATE_BANK', response.data.data)
        this.dialog = false
        this.clear()
      ...
      this.$swal?.fire({
        ...
        if (result.isConfirmed) {
          ...
          await this.$axios.delete(`${this.pageInfo.apiUrl}${id}`).then((response) => {
            this.$store.commit('accounts/bank/DELETE_BANK', index)
