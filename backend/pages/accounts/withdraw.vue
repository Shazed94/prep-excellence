<template>
  <div class="flex-grow-1">
    <bread-crumbs
      ...
      :items="[{text: 'Accounts',disabled: true, href: '/accounts'},{text: `${pageInfo.pageName}`,disabled: true, href: '/accounts/withdraw'}]"
    />
    <v-data-table
      dense
      v-model="selected"
    ...
    >
      <template v-slot:top>
        ...
        >
          <v-toolbar-title>
            <div>
              <export-excel :url="`withdraw/export?format=excel&ids=${selectedIds ? selectedIds : ''}`"
                            file_name="withdraw"/>
              <export-csv :url="`withdraw/export?format=csv&ids=${selectedIds ? selectedIds : ''}`"
                          file_name="withdraw"/>
              <export-pdf :url="`withdraw/export?format=pdf&ids=${selectedIds ? selectedIds : ''}`"
                          file_name="withdraw"/>
            </div>
          </v-toolbar-title>
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <!--                color="primary"-->
              <v-btn v-if="$can('create','Withdraw')"
                        ...
                        <v-col cols="12" xs="12" sm="6">
                          ...
                          >
                            <v-select
                              ...
                              :items="[{name: 'Hotel', val: 1},{name: 'Airline', val: 2},{name: 'Restaurant', val: 3}]"
...
<!--      </template>-->
      <template v-slot:item.type="{item }">
        {{ item.type == 1 ? 'Hotel' : item.type == 2 ? 'Airline' : 'Restaurant' }}
      </template>

      <template v-slot:item.actions="{ index, item }">
        <v-btn v-if="$can('update','Withdraw')"
          x-small
          class="mr-2"
          color="accent"
          fab
        ...
        <v-btn v-if="$can('remove','Withdraw')"
          color="error"
          x-small
          fab
...
export default {
  ...
  computed: {
    ...mapGetters('accounts/withdraw', ['withdraws', 'totalItems']),
  ...
  methods: {
    ...mapActions('accounts/withdraw', ['GET_ALL_WITHDRAWS']),
    payAbleAmountCount(Pax_list_id) {
      let data = this.paxById(Pax_list_id)
      this.formData.payable_amount = data.rate_per_day * data.number_of_day * data.passengers.length
    },
    async init() {
      this.loader.isLoading = true
      await this.GET_ALL_WITHDRAWS({
    ...
    toggleStatus(index, value, event, id) {
      this.$axios.get(`${this.pageInfo.apiUrl}/toggle/${id}`).then(response => {
        if (response.data.status === 'success') {
          this.$store.commit('accounts/withdraw/WITHDRAW_STATUS_CHANGE', {index, value})
    ...
    async submit() {
      ...
      await this.$axios.post(this.pageInfo.apiUrl, this.form).then(response => {
        this.dialog = false
        this.$store.commit('accounts/withdraw/ADD_NEW', response.data.data)
        this.clear()
    ...
    async update() {
      ...
      await this.$axios.post(`${this.pageInfo.apiUrl}/${this.selectedItem.id}`, this.form).then(response => {
        this.$store.commit('accounts/withdraw/UPDATE_WITHDRAW', response.data.data)
        this.dialog = false
        this.clear()
      ...
      this.$swal?.fire({
        ...
        if (result.isConfirmed) {
          ...
          await this.$axios.delete(`${this.pageInfo.apiUrl}/${id}`).then((response) => {
            this.$store.commit('accounts/withdraw/DELETE_WITHDRAW', index)
