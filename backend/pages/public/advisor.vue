<template>
  <v-container fluid>
    <bread-crumbs
      ...
      :items="[{text: `${pageInfo.pageName}`,disabled: true, href: '/public/advisor'}]"
    />
    <v-card>
      <v-data-table
        dense
...
const permission = 'Advisor'
const stateName = 'advisors'
const storeName='public/advisor'
export default {
  name: 'Advisor',
  head: {
    title: permission,
  ...
  computed: {
    ...mapGetters('public/advisor',['advisors','totalItems']),
    selectedIds() {
  ...
  methods: {
    ...
    createOrUpdate(item = null) {
      if (item) {
        this.editIndex = this.advisors.indexOf(item)
        this.editMode=true;
      }
      else {
        this.editIndex = -1
