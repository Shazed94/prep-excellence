<template>
  <v-container fluid>
    <bread-crumbs
      ...
      :items="[{text: `${pageInfo.pageName}`,disabled: true, href: '/public/gallery/category'}]"
    />
    <v-card>
      <v-data-table
        dense
      ...
      >
          ...
          >
            ...
            <v-dialog v-model="dialog" persistent max-width="600">
              <template v-slot:activator="{ on, attrs }">
                <!--                color="primary"-->
                <v-btn v-if="$can('create','Gallery Category')"
        ...
        <template v-slot:item.status="{ index, item }">
          <v-switch v-if="$can('status change','Gallery Category')"
                    class="pa-0 ma-0"
                    hide-details
                    :value="true"
        ...
        <template v-slot:item.actions="{index, item }">
          <v-btn x-small class="mr-1 accent" fab @click="createOrUpdate(item)" v-if="$can('edit','Gallery Category')">
            <v-icon>mdi-pencil</v-icon>
          </v-btn>
          <v-btn color="error" x-small fab @click="deleteItem(item, index, state.store_name)" v-if="$can('remove','Gallery Category')">
...
const permission = ''
const stateName = 'categories'
const storeName='public/category'
export default {
  name: 'category',
  ...
  computed: {
    ...mapGetters('public/category',['gallery_categories']),
    selectedIds() {
  ...
  methods: {
    ...
    createOrUpdate(item = null) {
      if (item) {
        this.editIndex = this.gallery_categories.indexOf(item)
        this.editMode=true;
      }
      else {
        this.editIndex = -1
