<template>
  <v-container fluid>
    <bread-crumbs
      ...
      :items="[{text: `${pageInfo.pageName}`,disabled: true, href: '/public/homePage/promotion'}]"
    />
    <v-card>
      <v-data-table
        dense
      ...
      >
        ...
        <template v-slot:item.status="{ index, item }">
          <v-switch v-if="$can('status change','Promotion')"
                    class="pa-0 ma-0"
                    hide-details
                    :value="true"
...
const storeName='public/promotion'
...
export default {
  ...
  computed: {
    ...mapGetters('public/promotion',['promotions','totalItems']),
    selectedIds() {
  ...
  methods: {
    ...
    createOrUpdate(item = null) {
      if (item) {
        this.editIndex = this.promotions.indexOf(item)
        this.editMode=true;
      }
      else {
        this.editIndex = -1
