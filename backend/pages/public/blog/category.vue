<template>
  <v-container fluid>
    <bread-crumbs
      ...
      :items="[{text: 'Blogs',disabled: false, href: '#'},{text: `${pageInfo.pageName}`,disabled: false, href: '#'}]"
    />
    <v-card>
      <v-data-table
        dense
...
const permission = 'Blog Category'
...
export default {
  ...
  methods: {
    ...
    createOrUpdate(item = null) {
      if (item) {
        this.editIndex = this.blog_categories.indexOf(item)
        this.editMode=true;
      }
      else {
        this.editIndex = -1
