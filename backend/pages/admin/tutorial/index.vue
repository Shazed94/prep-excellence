                        ...
                        <v-row dense>
                          ...
                          <v-col cols="12" md="6">
                            <validation-provider
                              v-slot="{ errors }"
                              name="category"
                              vid="tutorial_category_id"
                              rules="required"
                            >
                              <v-autocomplete
                                v-model="form.tutorial_category_id"
                                label="Category"
                                :items="tutorial_categories"
                                item-text="title"
                                item-value="id"
                                :error-messages="errors"
                                dense
...
const permission = 'Tutorial'
const stateName = 'tutorials'
const storeName='admin/tutorial/tutorial'
export default {
  name: 'Tutorial',
  head: {
    title: 'Tutorial',
  ...
  computed: {
    ...mapGetters('admin/tutorial/tutorial',['tutorials','totalItems']),
    ...mapGetters('admin/tutorial/category',['tutorial_categories']),
    selectedIds() {
  ...
  async mounted() {
    await this.init()
    const payload = {apiUrl: '/tutorial-category', stateName:'tutorial_categories'}
    await this.$store.dispatch('admin/tutorial/category/getItems', payload)
  },
  methods: {
    async init() {
    ...
    createOrUpdate(item = null) {
      if (item) {
        this.editIndex = this.tutorials.indexOf(item)
        this.editMode=true;
      }
      else {
        this.editIndex = -1
      ...
      if (this.editMode) {
        ...
        this.form = {
          title: item.title,
          tutorial_category_id: item.tutorial_category_id,
